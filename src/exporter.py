...
class EVMBlockExporter(FactExporter):
    ...
    def __init__(
        ...
        skip_sig_resolution: bool = False,
    ):
        super().__init__(output_dir)
        self.blocks = blocks
    ...
    def process_metadata(self, metadata: dict[Any, Any] | None = None) -> None:
        ...
        def process_function_debug_data(
            function_debug_data: dict[str, dict[str, int | None]],
        ...
        self.storage_layout = (
            metadata.get("storage_layout", {}) if metadata is not None else {}
        )
    ...
    def export(self):
        ...
        if os.path.isdir(COMMON_FACTS_DIR):
            try:
                os.symlink(
                    COMMON_FACTS_DIR, os.path.join(self.output_dir, "common-facts")
                )
            except FileExistsError:
                pass
